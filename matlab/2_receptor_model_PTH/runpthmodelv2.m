...
%
% L1 ... sensitized to desensitized state - give high PTH dosage cont.
% L2 ... densensitzed to sensitized state - give low  PTH dosage cont.
% L3 ... start from sensitized state - pulse high dose PTH: 1/2h on/off
% L4 ... start from sensitized state - pulse high dose PTH: different on/off
% L5 ... 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
elseif (LC==6)
  ...
  t_end  = (tau_on+tau_off)*4;     
  time = [t_init t_end];
  DPTH_vec = [0.015,0.1,0.5,1.0,5.0,7.5]; 
end
...
R_tot{j+(i-1)*length(tau_on)} =  R_T*length(x(:,2));
%
...
pi_3{j+(i-1)*length(tau_on)}  =  (x(:,3)) ./ (x(:,2)) ;

%
%
%
I_P = DPTH*360  % [pM/s]*360 0.00e+1; % 
k_5 = 2.00e-2; %  pM^{-1} day^{-1}
k_6  = 3.00e+0; % day^{-1}
k_P = 86;     % k_PTH
S_P = 250;    % S_PTH
...
%
plot(t_loadcase{j+(i-1)*length(tau_on)},Ra_loadcase{j+(i-1)*length(tau_on)},'-'), xlabel('t [hours]'), ylabel('concentration receptors [pM]')
hold on
