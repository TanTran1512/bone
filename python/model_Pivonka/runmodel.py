...
# add new parameters
BV_t0 = 1
y_t0 = [OBp_t0, OBa_t0, OCa_t0, BV_t0]
t_eval = np.linspace(0, t_end, t_end+1)
k_form = 0.3
k_res  = k_form*OBa_t0/OCa_t0
sol = solve_ivp(bonemodel, t_span=(0, t_end), y0= y_t0, 
                args = (para, t_1, t_2, LC, OBa_t0, OCa_t0), 
                t_eval = t_eval, method = "RK45", rtol = 1e-6, atol= 1e-6)

## Plot cell population over time
# Plot results
fig = plt.figure()
plt.plot(sol.t, sol.y[0], label="OBp")
plt.plot(sol.t, sol.y[1], label="OBa")
plt.plot(sol.t, sol.y[2], label="OCa")
plt.xlabel("t [days]")
